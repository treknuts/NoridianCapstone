...
class DataVisualization(Frame):
    ...
    def init_window(self):
        ...
        def set_filter_options(error_name, review, date1, date2):
            ...
            if review is not None:
                print(review)
                # Get the new data
                data_by_date = dv.get_errors_by_review_date(dv.errors, date1, date2)
                ...
                create_top_five(dv.errors, review)
                self.pie_frame.after(100, self.pie_frame.destroy())
                self.create_pie_chart(review)
                print("hit create_pie_chart")
            #elif review is None:
            ...
            if date1 != start_date.get_date() or date2 != end_date.get_date():
                print(date1 != start_date.get_date())
                print(date2 != end_date.get_date())

                print(date1, date2)
                data = dv.get_errors_by_review_date(dv.errors, date1, date2)
        ...
        oldest = list(dv.sort_by_date(dv.errors))[0]
    ...
    def create_pie_chart(self, review_level=None):
        ...
        if review_level is not None:
            ...
            top_five = dv.get_top_five_errors(dv.errors)
        labels = []
        sizes = []
